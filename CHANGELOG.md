...
- **A repository context file, `TETON.md`, read at the session root
  (REQ-612).** A `TETON.md` — or, failing that, an `AGENTS.md` — at a
  project's root is loaded when a session is created and on every `/cd`,
  re-read at the start of a prompt turn when it changes, and placed as the
  **last region of the system prompt** inside a `<repo-notes>` envelope framed
  as repository-authored description. A session knows the repository's shape
  without a walk. The block is capped at 8,192 bytes (or a quarter of the
  route's byte budget where that is smaller); past the cap the file is cut at
  a line boundary under a marker naming the bytes dropped, and the
  `repo_context_state` event announces every load, change and truncation —
  nothing is clamped in silence. The read is identity-checked by device and
  inode with `O_NOFOLLOW|O_NONBLOCK`; a symlinked, hardlinked or
  non-regular candidate is refused and the state says which.

  **Switches.** `[context] repo_file = true` in `config.toml` (the default)
  is the durable switch; `teton context enable|disable|status` is its shell
  surface. `/context on|off` switches the current session only and is never
  written; bare `/context` prints the state, the file, its size on disk, the
  resident bytes and the cap. A file a privacy boundary covers is withheld
  and the state says so — its bytes never reach the prompt.

- **Teton writes `TETON.md` when there is none (REQ-613).** The first prompt
  turn of a session in a project with no notes offers to draft them: at
  `guarded` it asks, at `plan` it is refused without a prompt, and at `full`
  it writes without one. `/context init [--force]` inside a session and
  `teton context init [--force]` from a shell are the explicit door; `--force`
  replaces an existing file by a scratch write and a rename, never a
  truncate. An `AGENTS.md`, or an empty `TETON.md`, is the documented way to
  say "no notes here" and stops the offer.

  **What the draft is made of.** One bounded walk of the whole tree under the
  tool walk's own budget (100,000 entries, ten seconds), the manifests and
  the README whole to 16 KiB each, the first 4 KiB of the entry-point files,
  and the workflow names — every path judged against the session's privacy
  boundaries before it is read, and the whole request held at the egress
  choke point like any other model call. The answer is bounded to the same
  8,192-byte cap, written under a `> Generated by Teton on <date>` header
  that names the tier and any budget stop, and read back by the loader the
  same turn so it is resident in the very request that offered it.

  **Consent is per root and per session.** The gate entry is keyed by the
  canonical root, remembered for the session, and forgotten on `/cd`; a
  decline is not asked again for that root. An unattended session (piped
  stdin, no client that can answer) is refused with one line and writes
  nothing — unless `[context] generate = always`, the automation opt-in,
  which writes into whichever project the session was launched in.
  `generate = never` stops the offer; `/context init` still writes.

  **The draft is its own category.** `draft` is the twelfth purpose category,
  bound to the `think` tier — a frontier provider by default. It is metered as
  one named `/cost` row and can be re-bound like any other with
  `teton policy set-category draft <provider-id>`.

### Changed

- **Upgrading raises one new question, once per project.** With the default
  `generate = ask`, the first prompt in a project that has no `TETON.md` puts a
  permission question on the terminal at `guarded`. Answer it, or set
  `[context] generate = never` before upgrading a machine that must not be
  asked. A piped or scripted session sees a one-line refusal and no write.

- **A repository's manifests, README and entry-point headers go to the
  `think` provider when a draft is written.** That is the same posture every
  `think` call already has — boundaries first, then the egress redaction scan —
  but it is a new payload. Bind `draft` to a local provider, or add a boundary
  over the files in question, if that provider must not see them.

- **Budget floors rose.** The route-aware context budget's floor moved from
  16,384 to 50,000 bytes (6,250 tokens), and the docs-topic ceiling moved to
  50,000 bytes with it. The resident-prompt ceiling the redaction scanner
  reserves for moved 14 → 23 KiB by measurement (four redaction chunks rather
  than three; the scannable bound rises to 184,265 bytes).

- **The wire protocol stays at version 2.** Every change since 0.1.29 is
  additive: the `repo_context_state` and `repo_context_generation` events, the
  `session/context` method, `ConfigUpdate::SetRepoContextEnabled` and
  `SetRepoContextGenerate`, the `draft` category and its `ConfigurableCategory`
  row, and the optional `context` posture in `config/get`. An older client
  that does not know `draft` will not see it unless it is bound.
