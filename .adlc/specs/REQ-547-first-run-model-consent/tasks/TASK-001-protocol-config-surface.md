...
- [x] All new payloads round-trip serde JSON and tolerate unknown fields (forward compat), matching the existing protocol test style
- [x] `model/confirm` params model the three outcomes as a closed enum — an unknown variant is a typed error, never a silent default
- [x] Config round-trips TOML; `auto_accept` defaults to **false** (BR-5 is opt-in); an invalid `pinned` name or malformed `base_url` fails validation with an actionable message
- [x] No transport/async code in teton-protocol; teton-core stays I/O-free (`cargo tree` shows no new I/O deps)
- [x] Tests: payload round-trips, closed-enum rejection, config default + validation cases
