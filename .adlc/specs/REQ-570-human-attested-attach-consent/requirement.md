...
- [x] AC-4: The CLI renders an incoming consent request, takes a decision, and sends `attach/consent`; it never answers without user input (asserted, including that a non-interactive CLI invocation does not auto-approve).
- [x] AC-5: An attestation is single-use and expires: replaying it, or using it for a different request id or connection, is refused.
- [x] AC-6: Attestation failure/cancel/timeout each produce a distinct code, mint nothing, and leave the grant and attestation registries empty (asserted by inspecting the registries, not inferred from the error).
- [x] AC-7: On a platform with no usable mechanism, cross-session attach is refused with the BR-8 posture code — never self-approved. Asserted by an injected "no mechanism available" seam so it is testable on any platform.
- [x] AC-7b: The Linux-without-a-polkit-agent case specifically reaches AC-7's refusal and names *that* cause — not a generic failure — and the refusal is reachable in CI on a headless Linux runner, which is the environment it actually describes. (BR-11)
- [x] AC-8: The single-client create → prompt → stream flow, and the creator's own attach, run with zero new prompts or attestation steps (the REQ-569 AC-7 regression bar, re-asserted).
- [x] AC-9: `grant_minted` carries the attestation method and is delivered to every handshaked connection.
- [x] AC-10: `model/confirm` and **each** of its six ungated siblings refuse an answer from a connection that did not raise the request — asserted per method at the raw RPC surface, not for one representative method (LESSON-502: an invariant enforced at several seams needs a test at each seam). A daemon-wide commitment additionally refuses when no valid attestation is presented. Layer (a) of this criterion passes independently of the attestation mechanism. (BR-10)
- [x] AC-11: Mutation check — making the attestation verifier return success unconditionally, dropping the single-use/expiry binding (BR-6), restoring the unattested self-approval routing arm (BR-3), or removing a method's connection check (BR-10a), each makes at least one test red. This REQ ships mostly *refusals*, and a refusal is verified only by proving it is load-bearing. (informed by LESSON-441)
