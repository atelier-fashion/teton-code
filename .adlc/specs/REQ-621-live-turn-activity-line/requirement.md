...
| TurnActivity | detail | string | optional; the model id and tier band for `awaiting_model` / `streaming`; the daemon-composed tool title for `tool_running`; for `held`, the one client-side composition the `turn_queued` notice also renders (BR-2 as amended 2026-09-10, ASSUME-049). Never composed by the client from raw arguments |
...
- [ ] BR-2: **Honest phase, daemon-supplied detail.** The row names the phase from a fixed vocabulary and fills detail only from event payloads: the provider, tier, and — when the event names one — the model from `route_decided`, the tool title the daemon composed from `tool_call`, and for a held turn **the same sentence the session's `turn_queued` notice renders**. The client never re-derives a title from raw arguments and never names a phase the daemon has not reported — before `route_decided` the row says it is preparing, not which model it is waiting on (informed by REQ-580 BR-7, LESSON-456). *(Amended 2026-09-10 at verify: the first draft said "the sentence the daemon wrote from `turn_queued`", which no daemon writes — the event carries a model id and a typed warming state, so the sentence is **composed by the client**. That is now one composition, `session_ui::tier_warming_clause`, which the durable notice and the row's `held until …` lead-in are two presentations of; two client-side compositions of one event are two sentences that agree until somebody edits one, and the row prints directly beneath the notice. Recorded as ASSUME-049.)*
...
- [ ] BR-5: **One row, repainted in place, gone without a trace.** The row occupies one terminal row, is repainted rather than appended, and never accumulates into scrollback. Durable lines that arrive during a turn (`shell: … [running]`, `[done]`, a permission prompt, a notice) print in their usual place and the row moves beneath them. When the turn ends the row is erased, so the session's scrollback after the turn is byte-identical to what it would have been without this feature, apart from BR-16's single verbose-mode line. **Two exceptions, recorded 2026-09-10 at verify, both bounded at one row:**
  - ~~a line the user **submits** while the row is animating moves the cursor a row down under bookkeeping a canonical-mode client cannot see, which makes every offset the row owns wrong; the row is abandoned where it is (BR-9), so that last frame stays in scrollback rather than a repaint or a withdraw landing on the line the user typed;~~ **Retired by REQ-622 on 2026-09-10** (REQ-622 BR-4, BUG-225 resolved). The exception's premise was that the client is in canonical mode for the length of a turn; REQ-622 takes the terminal out of canonical mode instead, so the kernel never moves the cursor, the row is never abandoned, and it is erased cleanly on every turn exit whether or not the user typed. This clause of BR-5 is now unconditional. The leg that recorded the exception (`pty_e2e::typed_bytes_survive_the_animation`) is deleted and its claims are made in the opposite direction by REQ-622's `a_submitted_line_is_never_overwritten_and_becomes_the_next_prompt`.  
  The second exception below is **not** retired: a stalled stream still breaks its paragraph where the row appeared.
  - a stream that stalls and resumes continues on a fresh line, because drawing the row flushes the held markdown line ahead of itself (ADR-621-3's consequence, `Surface::line`'s ordering rule since REQ-592 BR-8). Nothing is lost or duplicated; the paragraph is broken where the row appeared.
...
- [ ] BR-9: **Never blocks, delays, or consumes input.** The row's cadence adds no latency to receiving or rendering daemon events, and text typed while the row is animating is delivered intact to the next prompt; a repaint never blanks characters the user has echoed (informed by REQ-556 BR-4). *(Amended 2026-09-10 at verify — how the last clause holds, which the first draft left as an assertion. A line typed but **not** submitted is visually displaced while the row animates and reappears when the line is read: the kernel's buffer is untouched, which is the delivery half. A line **submitted** mid-turn is a different case, because the terminal's echoed newline has already moved the cursor: painting stops the moment a submitted line is waiting on stdin — the row is abandoned for the rest of the turn — so no repaint or withdraw can reach the row holding what the user typed. The cost is the frame BR-5 now records.)*
  **Retired by REQ-622 on 2026-09-10** (REQ-622 BR-3). The whole amendment above is a description of how the rule survives *the kernel* owning the line, and REQ-622 removes that owner: inside a turn the client echoes the pending line on a row it owns, so a typed line is not displaced, a submitted line does not move a cursor the client cannot see, and no painting has to stop. BR-9 holds unconditionally, and REQ-622 AC-1 is the leg that says so. The amendment is kept rather than deleted because it is the record of what canonical mode cost, which is the argument REQ-622 was written from.
...
- [ ] BR-13: **A rendering failure is never fatal and never silent.** A terminal error while painting the row abandons the row for the rest of the turn, leaves the turn itself untouched, and is recorded in verbose output (informed by REQ-556 BR-9, REQ-560 BR-13). *(Unchanged, and implemented 2026-09-10 at verify: the first implementation swallowed both row verbs' write errors, so the rule held only in the sense that nothing was fatal. `repaint_row_above` and `withdraw_row_above` now report whether their bytes were written and flushed, and the pump answers a `false` by hiding the row and — under `/verbose` — printing one line.)*
