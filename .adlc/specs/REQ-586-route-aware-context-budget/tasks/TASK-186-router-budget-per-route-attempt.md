...
- Tracer gotchas #1, #2, #9 (architecture.md). `Route.budget` is a copy of `harness.budget` — assert they agree. TASK-188 edits `runtime.rs` (`apply_update`, snapshot, setup) in disjoint regions and lands before this task — rebase onto it.
