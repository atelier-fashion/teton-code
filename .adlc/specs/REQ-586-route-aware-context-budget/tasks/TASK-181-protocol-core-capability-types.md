...
- `crates/teton-protocol/src/events.rs` — `BudgetBound { Window, DefaultUnknown, RedactScan, UserCap, LocalEngine }` and `ContextPressureKind { BlocksDropped, BlockElided, RefitOnReroute }` (`#[serde(rename_all = "snake_case")]`, `Copy`, `Eq`); `RouteDecided` (L364-410): `budget_tokens: Option<u64>`, `bound: Option<BudgetBound>` (default/skip; doc copies the `effort` additivity note); `Event::ContextPressure(ContextPressure { kind, dropped_blocks: u64, elided_bytes: u64, newest_user_elided: bool, budget_tokens: u64, bound: BudgetBound })` — **no `session_id`** (L2048-2059 flatten rule); `Event::name()` arm `"context_pressure"` (L189-222); rows in `event_names_match_the_spec_events_table` (~L2393) and the every-variant list (~L2527-2571); `context_pressure_round_trips_under_its_wire_name` copied from `context_cleared_round_trips_under_its_wire_name`; `route_decided_round_trips` (L2754) extended; skew test copied from `a_client_predating_the_cause_field_still_reads_a_frame_that_carries_one` (L2858).
...
- Placeholder arms so the workspace compiles: `crates/teton/src/session_ui.rs` `render_event` (L450-872, exhaustive) gets `Event::ContextPressure(_) => {}` with `// TASK-190 renders this`; every `RouteDecided { .. }` literal (`crates/tetond/src/harness/duty.rs:648` `DutyRoute::announcing`, `digest.rs:347`, `tools/grep.rs:928`, `harness/redact.rs:2877`, `router.rs:264-279`, `crates/teton/src/session_ui.rs:2553,2593,2632,2833`) gains `budget_tokens: None, bound: None`; every wire `ProviderConfig { .. }` literal (`tetond/src/runtime.rs:5359,11660,11747,11885,11919,12183`; `teton/src/main.rs:3417,4010,6179,6210`; `teton/src/cli_rows.rs:1560`; `teton/src/provider_test_ui.rs:861,881`; `tetond/tests/config_preservation.rs`) gains `max_context: None, context_budget_cap: None`; `ProviderRecipeEntry`/`ProviderSetupCandidate` literals in `crates/teton/src/provider_setup_ui.rs:1684-1727` and `tetond/src/provider_recipes.rs:324-337` get the field (recipe values are TASK-188's; use `0` here only in the projection so TASK-188's contract test turns red until it lands — say so in a comment).
