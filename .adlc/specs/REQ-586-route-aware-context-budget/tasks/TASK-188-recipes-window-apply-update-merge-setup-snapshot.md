...
- `crates/tetond/src/runtime.rs` — `apply_update` `RegisterProvider` arm (L9936-9964): `max_context: update.max_context.unwrap_or(existing.max_context)`, likewise `context_budget_cap` — preserve when `None`, write when `Some` (0 allowed = "unknown"); config→wire projection (L9785-9791): `max_context: Some(p.capabilities.max_context)`, `context_budget_cap: Some(..)`; `derive_provider_setup` (L5261-5366, esp. L5355-5366): the candidate's `max_context` — the recipe's window **only when the chosen model equals the recipe's `example_model`**, else `0` (unknown; the preview's `max_context = N` line shows which) — so a different model of the same vendor never inherits a wrong window; preview TOML shows the `max_context = N` line (tests at L21639/L22764 already assert such lines from fixtures).
