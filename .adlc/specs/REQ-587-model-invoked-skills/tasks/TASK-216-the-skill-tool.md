...
- [ ] **`READ_ONLY_TOOLS` carries `"skill"` as a bare literal** (TASK-215 had to — `SKILL_TOOL_NAME` ships with *this* task, and the comment there says so). Swap in the constant so the registry's name and the permission row cannot drift, and pin that they are the same value — nothing does today.
