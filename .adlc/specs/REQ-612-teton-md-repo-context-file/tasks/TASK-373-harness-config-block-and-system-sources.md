...
- `crates/tetond/src/harness/carry.rs` — `CarriedTurn::begin` and the reroute `rebudget` path
...
- [ ] AC-1: with `repo_context = Some(block)` the prompt ends with the block for both
      `HarnessConfig::default()` and `for_strong_model()`; with `None` it is byte-identical to
      the pre-task prompt.
- [ ] BR-5: `context_provenance` of a manager with a system source contains that id; a mutation
      that drops the union fails the test; the empty set contributes nothing (`is_unknown`
      false, `len` unchanged).
- [ ] LESSON-501: the source is present after `begin`, after `rebudget`, and after a replay
      from `RetainedContext` — three seams, three assertions.
- [ ] AC-9: two prompts with the same block hit the prefix cache; a changed block misses once.
...
| BR-5 | test-case | `crates/tetond/src/harness/carry.rs::system_sources_are_restated_at_begin_rebudget_and_replay` | no |
