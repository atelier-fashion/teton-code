...
- [ ] AC-4's "the substituted body reached the model" is asserted by TASK-204, from a fixture that drives `run_prompt_turn`. `context_pressure.rs`'s existing fixture seeds `CarriedTurn::begin` **by hand** (`:738`, "seeded exactly as `run_prompt_turn` seeds a turn"), so borrowing it would hand-build the expansion and assert it arrived — leaving the producer unguarded (LESSON-544). This suite asserts the **surface** bytes; the prompt content is asserted where it is produced.
