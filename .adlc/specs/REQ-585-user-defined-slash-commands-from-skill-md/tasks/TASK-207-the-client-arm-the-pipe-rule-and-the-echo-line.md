...
- [ ] `RefuseNoTerminal` and `RefuseUnrecognized` return `PermissionOutcome::Refused { reason }` (TASK-196) — **not** `Cancelled`, which already means "the user dismissed the prompt" and is what EOF on a pipe returns — **without calling `prompter.ask`**. The pin is negative and must be written as one: feed `/status\ny\n` on a pipe at `guarded` and assert `y` arrives as the **next prompt line**, not as an answer (`cli_e2e.rs:1830` is the template).
