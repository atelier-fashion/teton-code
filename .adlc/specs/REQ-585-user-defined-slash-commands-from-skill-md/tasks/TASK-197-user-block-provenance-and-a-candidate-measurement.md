...
- [ ] `ContextManager::would_seed_fit(system, text, budget_tokens, budget_bytes) -> Fit { tokens, bytes, fits }` is a **public associated function living in `context.rs`**, because `tokens_of`/`bytes_of` are private methods and `budget.rs` is a cross-module caller. It constructs a throwaway manager internally; that is safe because `ContextManager` has **no `Drop` impl** — the armed commit lives on `CarriedTurn` (`crates/tetond/src/carry.rs:362`), not on the manager. Measurement goes through the **existing** estimators, the same ones the pressure path uses; no second estimator is introduced (LESSON-546, LESSON-491).
