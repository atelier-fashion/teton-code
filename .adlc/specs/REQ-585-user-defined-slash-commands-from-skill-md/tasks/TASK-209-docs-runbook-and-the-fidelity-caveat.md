...
- [ ] Leg (f) and OQ-8's residual are stated plainly: on a boundary-configured machine every skill that runs a dynamic command pins its turn local, so all seventeen ADLC skills run on the local tier there. The consent offers no "run without dynamic context" in v1.
