...
- [ ] AC-1: `/help` order: built-in rows → blank → `skills — arguments are passed through as typed:` → one row per skill (`/name [hint] — description (user|project)`, shadowed marked) → the diagnostic line (`N skills (user A, project B); M skipped: …`) → blank → `ARGUMENT_FOOTER` → `ESCAPE_FOOTER`.
- [ ] `ARGUMENT_FOOTER` (`slash.rs:134`) is qualified to name the built-in rows it describes. It currently says arguments split on whitespace and quotes are not interpreted, which BR-4 makes false for skill rows; unqualified, it would sit two lines from the rows it contradicts.
...
- [ ] BR-3's both-directions pin: every name the snapshot classifies as `Input::Skill` appears as a `/help` row, and every `/help` skill row classifies as `Input::Skill` (LESSON-524).
