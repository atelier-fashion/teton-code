...
- [ ] `/help` order: built-in rows → blank → `skills — arguments are passed through as typed:` → one row per skill (`/name [hint] — description (user|project)`, shadowed marked) → the diagnostic line (`N skills (user A, project B); M skipped: …`) → blank → `ARGUMENT_FOOTER` → `ESCAPE_FOOTER`.
