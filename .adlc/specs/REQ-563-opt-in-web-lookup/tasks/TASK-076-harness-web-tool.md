...
- `crates/tetond/src/harness/turn_loop.rs` (same file as above) — feed `UserUrls` at user-prompt ingestion: every incoming user prompt's extracted URLs join the session set BEFORE the turn runs, so a paste-then-ask in one message classifies as UserPasted.
