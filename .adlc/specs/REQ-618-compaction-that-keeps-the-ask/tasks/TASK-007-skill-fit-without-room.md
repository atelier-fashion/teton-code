...
- [ ] `ROOM_FRACTION_PERCENT = 25`, integer arithmetic, one home; the verdict is
      `fits_without_room` when `body_bytes > room_fraction × budget_bytes` and the
      underlying `Fit` still fits.
- [ ] The refusal names the body size, the budget, the fraction and the remedy,
...
- [ ] A body at 30 % of the budget yields `skill_refused_no_room` and REQ-589's
      offer; `proceed once` expands **and anchors** it; `decline` ends the turn
      with no model call (AC-3).
- [ ] BR-2's second expansion: measured as
...
- [ ] Benign path: a body at 20 % expands untouched, with no offer and no event —
