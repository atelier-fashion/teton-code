...
- [x] BR-1: All four categories are tagged **at their call sites**. No prompt text,
...
- [x] BR-2: **Every duty emits `route_decided` — when it performs, not when it
...
- [x] BR-3: **Every duty's failure preserves the invariant its call site guards**
...
- [x] BR-4: **`compact` is the highest-risk duty and fails safe.** It decides what
      ...
      application.
- [x] BR-4a: **`compact` runs at a soft threshold; `truncate_to_budget` stays the
...
- [x] BR-4b: **`shell` interprets on failure or on oversized output, not on every
...
- [x] BR-5: Session taint overrides every duty binding, on all four, as it does for
      the turn path and `digest` (REQ-558 BR-7). Verified by egress capture.
- [x] BR-6: **The duty plumbing is generalised before the fourth caller lands.**
...
- [x] BR-7: **A duty's egress is scoped by the content it sends**, not by the turn's
...
- [x] BR-8: **A duty's output is bounded by the harness, not by the provider.**
...
- [x] BR-9: `title` runs **once per session**, not per turn, and never re-derives an
      existing title. It is `reflex`-tier and therefore local (REQ-558: `reflex`
      inherits the local tier and never `default_provider`).
- [x] BR-9a: **`title` reaches the wire in this REQ** (OQ-2, resolved) via
      ...
      not a pixel.
- [x] BR-11: **`policy show` states what content each category sends** (OQ-4,
...
- [x] BR-10: The `ScriptedFileEngine` duty-recognition seam gains an arm per duty.
...
- [x] AC-1: `teton policy show` renders **no** `declared, no call site yet` marker
...
- [x] AC-2: Each of the four emits `route_decided` naming its category, tier,
      provider, and a non-empty reason. `digest` does too (BR-2's retrofit).
- [x] AC-3: A table-driven test over all four duties × (resolves / unresolvable /
...
- [x] AC-4: Egress capture — with each duty bound to a remote provider and a
...
- [x] AC-5: A tainted session runs all four duties on the local tier regardless of
      binding, asserted by captured bytes (BR-5).
- [x] AC-6: `title` is requested exactly once for a multi-turn session, asserted by
      call count, and an existing title is never overwritten (BR-9).
- [x] AC-7: `compact` under a forced failure leaves the context under budget, with
      the degradation reported on the outcome (BR-4). A test asserts the "keep
      everything" fallback is not taken.
- [x] AC-8: One `DutyRoute`/`Duty` seam serves all four plus `digest`; a
      grep-level or type-level assertion pins that no per-category duty plumbing
      survives (BR-6).
- [x] AC-10: **Each duty is tagged at its call site** (BR-1): a compile-level or
      ...
      reintroduce it.
- [x] AC-11: **A duty's output is bounded by the harness** (BR-8): a remote
...
- [x] AC-12: **Every duty is answerable off-script** (BR-10): a scripted-engine
...
- [x] AC-13: **`shell` fires only on failure or oversize** (BR-4b): a table-driven
      ...
      *oversize* row red. It does not: truncation clamps the body to exactly the
      ...
      one.
- [x] AC-14: **`compact`'s soft threshold does not weaken the hard backstop**
      ...
      budget is enforced by `truncate_to_budget` and not by the duty.
      **Correction, found in Phase 5.** This AC originally claimed "a mutation
      that removes the unconditional 100% gate turns at least one of these red".
      It does not: all three arms call `truncate_to_budget` themselves, so they
      test that function's correctness rather than the loop's wiring. Deleting the
      loop's gate turns exactly **one** test red —
      `harness::turn_loop::tests::a_turn_whose_compact_duty_cannot_serve_still_
      ends_under_budget` — a test this AC does not name. The property is genuinely
      covered; the evidence sentence was wrong (LESSON-486: a claim that a
      mutation turns a named test red is a test, not a comment). Separately,
      making that gate *conditional* rather than deleting it leaves the whole
      suite green — an equivalent mutant at the loop, recorded in ADR-4 and
      coupled to BUG-157.
- [x] AC-15: **`session_titled` reaches the wire once** (BR-9a): a multi-turn
...
- [x] AC-16: **`policy show` names the content class per category** (BR-11): the
...
- [x] AC-9: Mutation checks — for each duty, (a) removing the taint override and
