...
- [ ] BR-6: **A lift releases opacity, and only opacity.** *(Amended 2026-09-05 by BUG-215; the original text follows.)* The `unknown` blocks already in context keep their provenance — nothing is rewritten — but a lifted session's remote sends are inspected with the `unknown` bit released: every minted source is still matched against the boundary globs, a `boundary_touch` sentinel still refuses, and a boundary read after a lift **escalates** the pin to `boundary_hit`, announced again with no remedy. What the lift asserts is exactly BR-4's sentence — *the command touched no protected file* — and the daemon honors that assertion at the one place it was being contradicted. The original rule read: "the `unknown` blocks already in context keep their provenance and are still refused at egress for the turn they are in; what the lift changes is that *later* turns are routed by category again. A carried `unknown` block from a lifted session that reaches a remote request is still blocked." As written, that made every later turn a remote route, a block and a local reroute — the lift moved `route_decided` and nothing else, and AC-3's "the next prompt routes remotely" was true only of the route line. Carried blocks are still re-inspected every turn (REQ-567); the inspection now reads the lift.
