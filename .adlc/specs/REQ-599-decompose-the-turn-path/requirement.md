...
- [x] AC-4 **(amended 2026-08-31, REQ-602 TASK-306)**: **Traceability check** — an automated check extracts every REQ/ADR/LESSON/BUG id from the pre-split files and asserts each is still **attached to the item it annotated**, wherever that item now lives, and that none has disappeared workspace-wide (BR-2). This check ships with the REQ and stays in CI.
  - **What changed and why.** As written, AC-4 also required failing "on an id that moved to an unexpected module" — and ADR-5 of this same REQ argues that clause is uncomputable: expecting a module for an id presumes ids belong to modules, which ADR-1 measured to be false. The criterion was ticked against the check that shipped, which has no module-ownership arm and never could. The amendment states the property that shipped and is verified: **re-attachment**, which is item-scoped and needs no module map. It is not weaker — it catches a comment left behind or captured by a neighbour, which a module-ownership rule would miss whenever the module was the expected one.
  - Per REQ-602 ADR-4 this is an amendment, not a re-tick: the original wording is recorded above so the change is reviewable rather than invisible.
...
- [x] AC-6 **(narrowed 2026-08-31, REQ-602 TASK-306)**: An event-sequence fixture recorded before the split replays identically after it, for a turn that exercises **a routing decision, a detached session-naming duty, streamed chunks, and a successful dispatch** (BR-1).
  - **What changed and why.** The original also named *skill expansion* and *a consent prompt*. `crates/tetond/tests/fixtures/req598_turn_event_order.txt` records neither: it is a plain typed prompt turn (`route_decided, session_titled, route_decided, session_update, prefix_cache`), captured at `17c39ec` before any `TurnContext` existed. The criterion was ticked on the fixture holding across all seven steps, which it did — for the scenario it actually contains.
  - **Why narrowed rather than met.** The gap is fixable in principle: `17c39ec` is still in history and still has the `carry_runtime`/`prompt` helpers, so a genuine pre-split capture is possible. It is not cheap: a turn that expands a skill *and* blocks on a consent prompt needs skill fixture files and a task answering the prompt, built against a historical harness. Regenerating the fixture at today's tip is not an option — a golden file computed by the subject it checks is not an oracle (LESSON-569), which is the property that makes this fixture worth anything.
  - **The gap is filed, not waved through**: REQ-604. What is uncovered is *ordering* for those two scenarios; the behaviours themselves are covered — skill-turn routing in `crates/tetond/tests/skill_turn.rs`, consent isolation in `crates/tetond/tests/skill_consent_matrix.rs` — neither of which pins where their events fall in a turn's sequence.
...
- [ ] AC-11 **(amended 2026-08-31, REQ-602 TASK-306 — NOT MET as written)**: Each commit in the sequence is independently green in CI (BR-9), demonstrated by the PR's commit-status history.
  - **What actually happened.** Five of the seven commits were fully green. On `f64d99b` (step 6) and `56f3777` (step 7) the `fmt · clippy · test (macos-latest)` job was **cancelled** — every other job on both commits succeeded (`feature-gated targets`, `cargo audit`, `release tooling`, `ubuntu-latest`, `catalog integrity`, `acceptance suite`). The cause is CI's `cancel-in-progress`: pushing the next step cancelled the previous commit's still-running macOS job.
  - **Why this is worth recording rather than waving through.** `macos-latest` is the runner that caught the last ordering defect in this very REQ — the detached-naming race behind LESSON-591 passed 40/40 locally and on `ubuntu-latest`, and failed only on macOS. Two steps therefore have no macOS evidence, on the one axis that has already produced a real failure here.
  - This is a claim about history and no present action can make it true, so it is amended rather than re-run. The steps are green **now**, at the tip, on every runner — which is a different and weaker claim than BR-9's.
...
| AC-4 | **Amended** (REQ-602 TASK-306). Satisfied by **extending** REQ-598's sweep rather than inventing a module map to assert against (ADR-5). Its re-attachment arm caught a real orphaning in step 3. The module-ownership clause was dropped from the criterion because ADR-5 shows it is uncomputable — it was never implemented and the original tick did not say so. |
| AC-5 | The sweep's mutations were run in REQ-598 and re-run here; step 3's failure *was* the live demonstration. |
| AC-6 | **Narrowed** (REQ-602 TASK-306). The event-ordering fixture held across all seven steps — for a plain typed turn. It contains no skill expansion and no consent prompt, both of which the original criterion named. Filed as REQ-604. |
...
| AC-11 | **NOT MET, amended** (REQ-602 TASK-306). Five of seven fully green. On `f64d99b` and `56f3777` the `macos-latest` job was cancelled by `cancel-in-progress`; all other jobs green. macOS is the runner that caught LESSON-591's race, so the gap is on the axis that has already failed here. |
