...
| setup commit (method name per OQ-1) | the session's own user connection **and** presence-attested where the platform can attest (REQ-575/576 pattern); a model tool call or foreign connection gets `NOT_ATTACHED` in the response and `provider_setup_rejected_nonuser` as an event (BR-12, as amended) |
...
- [ ] BR-12: **User-only, presence-attested where possible.** All three setup methods refuse model tool calls and non-session connections *in the response* with the existing `NOT_ATTACHED` wire code (`-32009`, REQ-568 BR-4) — the same code `web/setup_*` refuses with, not a new one; only the commit refusal is additionally announced as the `provider_setup_rejected_nonuser` **event**, because a pre-authorization publish on a read-only method is attacker-paced. The commit sits behind the same presence gate as `web/setup_commit` and `config/set`, and degrades exactly as they do on a build without the presence feature — never more permissively. *(informed by REQ-570, REQ-575, REQ-576; amended after implementation — the original text said "a distinct wire code", which would have meant minting a new one for a question `NOT_ATTACHED` already answers; the refusal is deliberately the existing code plus a distinct event, matching `web/setup_*` exactly. The distinctness that matters is the event's, and it is the commit's alone.)*
...
- [ ] BR-14: **Replacing an existing provider is explicit.** If the chosen id already exists, the flow says so, shows what would change, and asks; a key rotation restores the prior key on refusal. A **typed** id that already exists is named before the model prompt — the warning keys off the id the user chose, never the recipe's suggestion, so a user who typed over a colliding suggestion is not warned about a provider they are not replacing. Silent replace-or-insert is the BUG-155 class and is not permitted here. *(informed by REQ-557, BUG-171; the typed-id clause added by owner decision after Phase 5, with the id prompt)*
...
- [ ] AC-2: `/provider setup kimi` in a TTY session walks vendor → id (default the recipe's suggestion) → model (default `kimi-k3`, labeled example) → key (echo-off) → routing (default `think`) → preview → confirm, and on `y` the same session's next `think`-classified turn routes to `kimi`. No restart, no re-attach. *(amended by owner decision after Phase 5 — the id prompt was added so a second provider for a vendor the machine already holds is reachable in-session; Enter keeps the suggestion, so the walk this AC describes is unchanged for the user who takes every default.)*
...
- [ ] AC-7: Cancel at each of the six prompts leaves config byte-identical and the keychain without a `provider:kimi` entry — asserted by reading both artifacts. *(five before the id prompt was added; the test truncates the answer script one answer at a time, so the count follows the walk)*
...
- [ ] AC-10: A model tool call naming the setup-commit method, and a setup-commit from a second connection that did not open the session, are both refused **in the response with the existing `NOT_ATTACHED` wire code** *and* announced with the **`provider_setup_rejected_nonuser` event**, and no config or keychain change occurs. Plan and preview refuse the same callers in-response only, with no event (LESSON-513). (Method naming per OQ-1.) *(amended after implementation — the original text named `provider_setup_rejected_nonuser` as the "wire code", conflating the response code with the event; there is no new wire code, deliberately, because `web/setup_*` refuses the identical question with `NOT_ATTACHED` and a second spelling for one method would be a divergence, not a signal. The tests document the split.)*
