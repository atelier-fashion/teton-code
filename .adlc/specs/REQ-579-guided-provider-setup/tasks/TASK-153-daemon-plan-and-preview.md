...
- `crates/tetond/src/server.rs` — `handle_provider_setup_plan`, `handle_provider_setup_preview` beside the `handle_web_setup_*` fns; add both to the sync `dispatch()` match; both gated by `refuse_unmintable_session_id` + `may_drive`; foreign caller → `SETUP_REJECTED_NONUSER` in the response, no event
...
- [ ] `provider/setup_plan` from the session's own connection returns catalog (== recipes), existing ids, tiers; from a foreign connection returns `SETUP_REJECTED_NONUSER` and emits no event
