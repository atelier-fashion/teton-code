...
- [ ] BR-5 **(amended 2026-09-01, REQ-607)**: No credential *value* and no withheld variable *name* appears in tool output, error text, logs, or any event payload (informed by REQ-562 BR-6 — findings carry spans and kinds, never the matched text).
  - **What changed and why.** As written this rule is unconditional, and REQ-607's failure advisory names `SSH_AUTH_SOCK` in a `shell` tool result — a withheld variable name, in tool output. That is a narrowing of BR-5, and REQ-607 records it here rather than asserting upstream that nothing moved. The **name** half now reads: no variable name **discovered from the daemon's live environment**, and no name resolved from a configured `auth_ref = "env:<NAME>"`, appears in tool output, error text, logs, or any event payload. Names drawn from the statically documented rejection table in `child_env.rs` are exempt — they are public in the source, identical on every installation, and disclose nothing about *this* machine, which is the property BR-5 exists to protect. The **value** half is untouched and unconditional.
  - **Status (updated 2026-09-01).** REQ-607 has **shipped**, so the narrowed rule above is the one now enforced. The `shell` tool names `SSH_AUTH_SOCK` in the advisory it appends to a failing command that the withholding plausibly explains — one sentence, on the failing call, and only when the daemon really had the variable and the child really did not. Everything else BR-5 forbids is unchanged and still enforced: no name discovered from the live environment, no name from a configured `auth_ref = "env:<NAME>"`, and no credential value under any condition. (This bullet previously read that REQ-607 was `draft` and BR-5 was enforced as originally written; that was true until the flag landed, and leaving it would have made this rule read one way here and another in REQ-607.)
  - **What the amendment does not license.** It is not a general permission to describe the withheld set. OQ-1's settlement stands: no event carrying a count or a list. The exemption reaches one sentence, on one failing call, naming one statically-known variable.
