...
- [ ] BR-5 **(amended 2026-09-01, REQ-607)**: No credential *value* and no withheld variable *name* appears in tool output, error text, logs, or any event payload (informed by REQ-562 BR-6 — findings carry spans and kinds, never the matched text).
  ...
  - **Status.** REQ-607 is `draft` as of this amendment. Until it ships, BR-5 is enforced as originally written and the shipped code discloses no withheld name at all; the amendment is recorded now so the rule is not narrowed downstream while reading unchanged here.
