...
- [ ] BR-5 **(amended 2026-09-01, REQ-601)**: No credential *value* and no withheld variable *name* appears in tool output, error text, logs, or any event payload (informed by REQ-562 BR-6 — findings carry spans and kinds, never the matched text).
  - **What changed and why.** As written this rule is unconditional, and REQ-601's failure advisory names `SSH_AUTH_SOCK` in a `shell` tool result — a withheld variable name, in tool output. That is a narrowing of BR-5, and REQ-601 records it here rather than asserting upstream that nothing moved. The **name** half now reads: no variable name **discovered from the daemon's live environment**, and no name resolved from a configured `auth_ref = "env:<NAME>"`, appears in tool output, error text, logs, or any event payload. Names drawn from the statically documented rejection table in `child_env.rs` are exempt — they are public in the source, identical on every installation, and disclose nothing about *this* machine, which is the property BR-5 exists to protect. The **value** half is untouched and unconditional.
  - **Status.** REQ-601 is `draft` as of this amendment. Until it ships, BR-5 is enforced as originally written and the shipped code discloses no withheld name at all; the amendment is recorded now so the rule is not narrowed downstream while reading unchanged here.
