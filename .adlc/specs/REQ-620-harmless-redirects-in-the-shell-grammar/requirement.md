...
Five things this REQ knowingly did not do. None is a gap in what it claims;
...
- **A redirect glued to its *verb* stays `unknown`, by design.**
  ...
  at a home root, which the pre-REQ-620 gate allowed.)* **The peel is one level
  deep** (recorded at the Phase-5 re-verify): the tail is emitted as a single
  word and never handed back to the strip's loop, so a second redirect glued
  behind the separator is not peeled — `ls 2>&1;&>/dev/null` peels the `2>&1`,
  re-emits `;&>/dev/null` whole, and stays `unknown` on the surviving `>`.
  Recursing would mean re-splitting a word `sh` reads as one token, which is
  the lexer ADR-614-1 refuses to become; the shape appears in no transcript,
  and the rule is written down so a later reader does not file it as a defect.
  The spaced form is stricter still: the operator word must be bare, so `>|`,
  `2>&` and `2>;` never lift a following `/dev/null`. Every miss lands on the
  old answer, which is the property that keeps BR-1's widening provable;
  relaxing the whole-word rule is what mutation 2 in `shell_syntax.rs`'s record
  shows going red, and relaxing the spaced arm is mutation 6.

- **A pager on the end of a pipe keeps the root walk.** `less` and `more` were
  on BR-4's allowlist and left it at the Phase-5 re-verify. Neither is a pure
  filter: both take commands from the terminal once running (`:e path` opens
  another file, `!cmd` shells out) and `less` honours `LESSOPEN`, an
  environment-set preprocessor that runs on whatever it is handed. None of that
  is visible to a grammar reading the command's text, and a pager at the end of
  a pipe is not a shape a model writes for output it means to consume — so
  `ls | less` walks the root, which is the pre-REQ-620 answer. Modelling a
  pager's own command language is a different requirement and not a small one.
