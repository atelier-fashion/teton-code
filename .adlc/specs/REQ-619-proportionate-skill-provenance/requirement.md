...
- [ ] AC-6: the exit-code channel: a preamble `` grep -q MARKER secrets/prod.env && exit 1 || exit 2 `` pins with `boundary_hit` **and** the placeholder the fold writes into the prompt never reaches the mock — the verdict is taken before the spawn, so the turn is refused whatever the exit code was (BR-1, BR-2). The claim holds for spellings the grammar parses; a whole-command short-circuit (`>/dev/null; exit $?`) is `unknown_shell`, liftable, per BR-2's clause.
...
- [ ] AC-8: a user skill's identity survives the carry and a client re-attach: after the skill turn is committed on one connection, a boundary glob added over the skills directory mid-session refuses a **second client's** next send naming the file (BR-5). The compaction hop is proved in-process at the seam (`compaction_summary` carries the id and the boundary bit) rather than end to end — forcing window-pressure compaction from a client means sizing a fixture against a provider window, the LESSON-640 shape — and the e2e test says so in its doc. *(Amended 2026-09-05 at verify: the original text asked for compaction end to end.)*
...
- REQ-614's judgment that a name-only verb (`ls -R`, `find`, `du -a`) under a protected directory is `rooted` — names and sizes, never bytes — stands; a preamble inherits it. Whether a `SKILL.md` writable by anything with `~/.claude/skills` access should be held to a stricter rule than a model's `shell` call is a separate question. *(Noted 2026-09-05 at verify.)*
- The egress inspector's arm order (the unknown sentinel is reported before a matched boundary source). Filed as BUG-216; this REQ pins the current behaviour with a test so the change is recorded when it lands.
