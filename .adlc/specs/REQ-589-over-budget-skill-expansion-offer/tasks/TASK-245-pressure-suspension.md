...
- [ ] Both calls are skipped on the first iteration of an accepted turn and on no other
- [ ] The flag clears before the second iteration and cannot leak to the next turn (D-7)
- [ ] With history large enough to trigger the gate, no block is dropped; the block list before and after is compared (AC-16)
- [ ] A turn that then cannot fit surfaces the typed context outcome rather than a silently shortened conversation
- [ ] Deleting the suspension reddens a dedicated seam test (LESSON-508) — end-to-end coverage alone is not sufficient
