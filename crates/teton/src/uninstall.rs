...
impl Plan {
    ...
    pub fn build(paths: &DaemonPaths, keep_data: bool, brew_prefix: Option<PathBuf>) -> Self {
        let state_dir = paths
            .socket
            .parent()
...
fn execute(paths: &DaemonPaths, plan: &Plan, surface: &mut dyn Surface) -> anyhow::Result<()> {
    ...
    if plan.brew_prefix.is_some() {
        ...
        surface.line(
            LineKind::Notice,
            "done. If you are finished with the tap as well: brew untap atelier-fashion/tap",
        );
    } else {
        surface.line(
            LineKind::Notice,
...
mod tests {
    ...
    fn plan_names_the_size_and_every_step() {
        ...
        let plan = Plan::build(&paths, false, Some(PathBuf::from("/opt/homebrew")));
