...
impl DaemonRuntime {
    ...
    async fn settle_dynamic_context(
        ...
        let runs: Vec<crate::skills::dynamic::PreambleRun> = match door {
            ...
            // M1).** Nothing here spawns a process, which is what made an inline
            ...
            // may carry `MAX_DYNAMIC_COMMANDS` of them. A declined invocation is
            // therefore capable of parking an async worker for tens of seconds
            // and stalling every other session on it, which is precisely the
            // reason the consent-given arm is not inline either. The door being
            // shut changes how long the work takes, not where it belongs.
            Some(reason) => {
                let reach = reach.clone();
