...
/// The widest a [`generated_header`] line may be (REQ-613 ADR-5).
///
/// A budget, not a defence: the header is charged against REQ-612's cap before
/// the model's answer is bounded into what is left, so it has to be a figure the
/// bounding step can subtract with certainty rather than a length the composer
/// discovers afterwards.
pub const GENERATED_HEADER_MAX_BYTES: usize = 200;

/// The one line a Teton-generated `TETON.md` opens with (REQ-613 BR-6, ADR-5).
///
/// Four facts and a sentence: the date, the tier that served the draft, whatever
/// the evidence walk had to leave out (`stop` where the walk hit its budget,
/// `cut` where the tree was cut), and the invitation to edit. The invitation is
/// the load-bearing half — a generated file that reads as machine property gets
/// left to rot, and the point of BR-6's header is that the next person to open
/// it knows it is theirs to rewrite and that Teton will read what they write.
///
/// Composed here rather than in `write.rs` because it is a *rendering* — a
/// function of four strings and nothing else, testable against a golden with no
/// filesystem behind it, in the module that already owns what the block's bytes
/// look like.
///
/// # Why the bound is the function's and not the caller's
///
/// [`GENERATED_HEADER_MAX_BYTES`] holds by construction, in two steps. A line
/// that would exceed it is recomposed **without the optional facts** — they are
/// the only unbounded part in practice and the only part the sentence can lose
/// and still be true. If even that is over — reachable only from a `tier` or a
/// `date` far outside anything this build produces, since a closed tier name and
/// an ISO date compose to 110 bytes — the line is cut at a character boundary.
/// Both steps exist so the ≤ 200 claim is a property of this function; a bound
/// enforced only by "the callers pass short strings" is a bound that a later
/// caller silently repeals.
#[must_use]
pub fn generated_header(tier: &str, date: &str, stop: Option<&str>, cut: Option<&str>) -> String {
    // `stop` then `cut`, the order the walk reaches them: a walk that ran out of
    // budget and a tree that was cut are two different omissions and a reader
    // who sees both should read them in the order they happened.
    let mut facts = String::new();
    for fact in [stop, cut].into_iter().flatten() {
        facts.push_str("; ");
        facts.push_str(fact);
    }

    let compose = |facts: &str| {
        format!(
            "> Generated by Teton on {date} ({tier} tier{facts}). \
             Edit freely — Teton reads this file at every session start."
        )
    };
    let mut line = compose(&facts);
    if line.len() > GENERATED_HEADER_MAX_BYTES {
        line = compose("");
    }
    if line.len() > GENERATED_HEADER_MAX_BYTES {
        let mut end = GENERATED_HEADER_MAX_BYTES;
...
mod tests {
    ...
    /// REQ-613 BR-6 / ADR-5: the header is one line naming the date, the tier
    /// and whichever of the two omissions happened, inside 200 bytes.
    ///
    /// The goldens are hand-typed, not composed from the pieces the function
    /// composes from — an expectation built by the subject asserts nothing
    /// (LESSON-569).
    ///
    /// Mutation, each one run and watched go red: reordering `stop` and `cut`
    /// fails the both-facts golden; narrowing the `; ` separator to a space
    /// fails it too; deleting the drop-the-facts step fails the overlong leg,
    /// and deleting the char-boundary cut fails the absurd-tier leg — which is
    /// why that leg is here rather than left to the callers' good manners.
    #[test]
    fn the_generated_header_names_the_tier_the_date_and_the_omissions_in_one_short_line() {
        let cut = generated_header("think", "2026-09-03", None, Some("tree cut at depth 6"));
        assert_eq!(
            cut,
            "> Generated by Teton on 2026-09-03 (think tier; tree cut at depth 6). \
             Edit freely — Teton reads this file at every session start."
        );

        let both = generated_header(
            "think",
            "2026-09-03",
            Some("walk stopped at 100,000 entries"),
            Some("tree cut at depth 6"),
        );
        assert_eq!(
            both,
            "> Generated by Teton on 2026-09-03 (think tier; walk stopped at 100,000 entries; \
             tree cut at depth 6). Edit freely — Teton reads this file at every session start."
        );

        let plain = generated_header("local", "2026-01-31", None, None);
        assert_eq!(
            plain,
            "> Generated by Teton on 2026-01-31 (local tier). \
             Edit freely — Teton reads this file at every session start."
        );

        // One line, always: the header is line one of a Markdown file and a
        // second line in it would be a second thing the loader frames.
        for header in [&cut, &both, &plain] {
            assert!(!header.contains('\n'), "the header is more than one line");
            assert!(
                header.len() <= GENERATED_HEADER_MAX_BYTES,
                "the header is {} bytes",
                header.len()
            );
        }

        // The bound is the function's. A fact far past the budget costs the
        // facts, not the sentence...
        let overlong = generated_header("think", "2026-09-03", Some(&"stopped ".repeat(30)), None);
        assert_eq!(
            overlong,
            "> Generated by Teton on 2026-09-03 (think tier). \
             Edit freely — Teton reads this file at every session start."
        );
        assert!(overlong.len() <= GENERATED_HEADER_MAX_BYTES);

        // ...and a tier no route can produce still cannot spill past the cap.
        let absurd = generated_header(&"t".repeat(500), "2026-09-03", None, None);
        assert_eq!(absurd.len(), GENERATED_HEADER_MAX_BYTES);
