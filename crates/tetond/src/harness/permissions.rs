...
/// does not exist — a prompt carrying two different durable writes, or an
/// acknowledgment that also persists a web tier. The enum makes those
/// unrepresentable rather than merely unreachable.
enum Question {
    /// Every caller but the acknowledgment: the four standard options, plus
...
impl PermissionGate {
    ...
    async fn settle_skill_consent(
        ...
        match self
            .settle(key, None, question, Some(addressed), level_allow)
            .await
        {
    ...
    /// lookup. [`Self::remembered`] can only ask about a key someone already
    /// thought of, which is not the same question: a skill's grant has two
    /// legal spellings ([`skill_grant_key`]: the plain `skill:<source>:<name>`
    /// and that plus a digest of its substituted commands), so a grant left
    /// behind under one is invisible to a lookup at the other and fully visible
    /// to the door that consults it. See
    /// [`a_permanent_acknowledgment_frees_only_the_acknowledgment_key`](tests::a_permanent_acknowledgment_frees_only_the_acknowledgment_key).
    #[cfg(test)]
...
mod tests {
    ...
    ///
    /// Separate behaviours because the ways a question can fail to reach a
    /// human are **different things**: a single "does not answer" double would
    /// collapse a decline, a dismissal and *nobody was there to ask*, and D-13's
    /// widening turns on the last of those alone.
    enum RouteBehaviour {
    ...
    /// A gate with an addressed route wired into it, plus the bus, the route,
    /// and the connection the question is addressed to.
    fn wired(
        ...
        let bus = Arc::new(EventBus::new());
        let pending = Arc::new(PendingPermissions::new());
        let route = Arc::new(AddressedRoute {
