...
The name is the directory name or file stem (`^[a-z0-9][a-z0-9_-]{0,63}$`), not
a frontmatter `name`. Built-in beats skill, project beats user, `skills/` beats
`commands/`; the loser is shadowed and does not dispatch. Discovery re-runs on
`/cd`; there is no watcher. `CLAUDE.md`, agents and hooks are **not** loaded.

`/help` lists what registered and names anything skipped and why: the 64 KiB
cap, unreadable, malformed, badly named.
...
A typed `/name` is from there an ordinary prompt: same classifier, routing,
level, egress gate, cost row.

The tool's description names what the model may invoke; a call with no name
lists them. It is the only path to a body outside the root; `read` stays jailed,
companion files included.
...
`user-invocable: false` makes it model-only (`/help` marks it `(model-only)`).
A non-boolean value reads as the safe one — user only. Every other key
(`allowed-tools`, `model`, `agent`, `hooks`, …) is inert, listed by `/verbose`.
A body saying "run this at `full`" is a sentence, not a setting.

## Dynamic context — `` !`cmd` ``

A body may inline a command's output at expansion; substitution runs first, so
the command is consented to as it will run. It runs under the skill's **own**
key, `skill:<source>:<name>`, never `shell`'s — no grant crosses, and project
grants drop on `/cd`. Default: `guarded` and `edits` ask once per invocation
with each command shown; `plan` does not run them, `full` does. On piped stdin
at a level that would ask, the client refuses without reading it: the next line
is a prompt, never a `y`.

Commands run in document order, session root as cwd, under `shell`'s jail,
timeout and output cap. One that did not run leaves
``[dynamic context not run: `cmd` — reason]`` in its place.
...
Nothing is translated or rewritten. `Agent`, `Task`, `Workflow`, subagents and
Claude Code tool names pass through with nothing behind them. A skill that
invokes other skills now runs them — that is what the `skill` tool is for — but
one that dispatches subagents degrades to this one loop. Say so plainly rather
than pretend a step ran.
